setwd("E:\\stats\\reefmap.pq.2013")

t.photoquadrats_20150721 <- read.delim("E:/stats/reefmap.pq.2013/t.photoquadrats_20150721.txt", row.names=1)
...
df.others.zoo<-as.data.frame(apply(df.reef.pq[,c('fire.coral','other')],1,sum))
...
#reshape the simplified dataframe for plotting

df.reef.pq.summary <- melt(df.reef.pq.simplified,measure.vars=c('sand','limestone.bedrock','hard.coral','rubble','seagrass','soft.coral','others.zoo','dead.standing.hard.coral','dead.standing.hard.coral.ta','crustose.coraline.algae','macro.algal.assemblage','seagrass','sponge'))
...
# Melt reshaping further simpification to compare with AMP results
#AMP categories: (Sand, rock, rubble),Algae,Hard coral,Soft coral, Dead coral
...
#GROUPING 3: sand,coral rock, and rubble

        df.srr<-as.data.frame(apply(df.reef.pq[,c('sand','rubble','limestone.bedrock','silt','rock')],1,sum))
        colnames(df.srr) <- "sand.limestone.rock.rubble"
#assign index
...
df.reef.pq.simplified3$sand<-NULL
...
#reshape the simplified dataframe for plotting
df.reef.pq.AMP <- melt(df.reef.pq.simplified3,measure.vars=c('hard.coral',"sand.rock.rubble",'seagrass','soft.coral','others.zoo','dead.coral','crustose.coraline.algae','macro.algal.assemblage','sponge'))
p.AMP <- ggplot(df.reef.pq.AMP) + geom_boxplot(aes(x='Simplified benthic cover',y=value,color=variable)) + labs(x = "Simplfied benthic cover type",y = "% Cover", title = "Summary of benthic habitat type cover from photoquadrat analysis")
p.AMP
p.region <- ggplot(df.reef.pq.AMP) + geom_boxplot(aes(x=region, y=value, color=variable)) # SELECTED VARIABLES
p.region
p.aspect <- ggplot(df.reef.pq.AMP) + geom_boxplot(aes(x=aspect, y=value, color=variable)) # SELECTED VARIABLES
p.aspect
p.depth <- ggplot(df.reef.pq.AMP) + geom_boxplot(aes(x=depth, y=value, color=variable)) # SELECTED VARIABLES
p.depth
...
boxplot(coral ~ depth,data = df.reef.pq,col = colours,main="Percentage cover", xlab = "Depth", ylab = "Coral") # CORAL
